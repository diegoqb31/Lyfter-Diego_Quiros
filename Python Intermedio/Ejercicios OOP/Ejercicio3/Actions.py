    ...
    while True:
        try:
            ...
            if new_float < 0:
                ...
                print("El campo no puede ser mayor a 100.")    
            else:
                return new_float
...
def add_student(students_list):
    ...
    new_student = Student(student_name,last_student_name, student_group, spanish_grade,english_grade, social_grade, science_grade)
    
    new_student.average = calculate_average(new_student)
    
    if not all([student_name, last_student_name, student_group]):
        print("Hay campos en blanco. No se agregó el estudiante.")
        return
    
    students_list.append(new_student)
    
    print(f"Estudiante {new_student.name} agregado correctamente.")
...
def top3_grades(students_list):
    ...
    students_list.sort(key=lambda s: s.average, reverse=True)
    students_list = students_list[:3]
    
    calculate_and_show_averages(students_list)